    ...
    if platform.system() == "Linux":  # Ensure function only runs on Linux systems
        ...
        try:
            ...
            if os.path.exists(audit_path):  # Check if audit.txt exists
                ...
                print(bcolors.RED + bcolors.BOLD + "[!] audit.txt not found — carving might have failed or no files were found" + bcolors.ENDC)
...
def detect_rogue_dhcp_servers():
    ...
    for (ip, mac), data in dhcp_servers.items():
        if ip in whitelist_ips or mac in whitelist_macs:
            ...
            status = "[!] Not in whitelist → Possible Rogue Server"
            rogue_count += 1
    ...
    if not suspicious_ips:
        ...
        for ip in suspicious_ips:
            ...
            if creds:
                print(bcolors.YELLOW + bcolors.BOLD + f"\n-> {ip}:" + bcolors.ENDC)
                for line in creds:
                    print(bcolors.RED + bcolors.BOLD + f"    → {line}" + bcolors.ENDC)
            else:
...
def detect_mac_spoofing():
    ...
    for mac, ips in mac_to_ips.items():
        if len(ips) > 1:  # More than one IP mapped to same MAC → possible spoofing
...
def get_all_usernames_passwords():
    ...
    pop3_state = {}  # session (ip_src, ip_dst) → {"user": ...}
    ...
    for packet in pcap_file:
        ...
        if "Authorization: Basic" in payload:
            ...
            if m:
                ...
                try:
                    ...
                    credentials["HTTP"].append(bcolors.BLUE + bcolors.BOLD + f"{ip} → Basic Auth: " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{decoded}" + bcolors.ENDC)
                except:
                    credentials["HTTP"].append(bcolors.BLUE + bcolors.BOLD + f"{ip} → Basic Auth (Invalid): " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{b64}" + bcolors.ENDC)
                stats["HTTP"] += 1
        ...
        if "POST" in payload and "log=" in payload and "pwd=" in payload:
            ...
            if u and p:
                ...
                credentials["HTTP"].append(bcolors.BLUE + bcolors.BOLD + f"{ip} → POST user: " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{u.group(1)}" + bcolors.ENDC)
                credentials["HTTP"].append(bcolors.BLUE + bcolors.BOLD + f"{ip} → POST pass: " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{p.group(1)}" + bcolors.ENDC)
                stats["HTTP"] += 2
        ...
        if "USER " in payload:
            ...
            credentials["FTP"].append(bcolors.BLUE + bcolors.BOLD + f"{ip} → USER: " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{username}" + bcolors.ENDC)
            stats["FTP"] += 1
        if "PASS " in payload:
            password = payload.split("PASS ")[1].split("\r")[0]  # Extract password
            credentials["FTP"].append(bcolors.BLUE + bcolors.BOLD + f"{ip} → PASS: " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{password}" + bcolors.ENDC)
            stats["FTP"] += 1
        ...
        if packet.haslayer(TCP) and (packet[TCP].dport == 110 or packet[TCP].sport == 110):
            ...
            for line in lines:
                if line.upper().startswith("USER "):
                    ...
                    entry = bcolors.BLUE + bcolors.BOLD + f"{ip} → USER: " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{username}" \
        ...
        if packet.haslayer(TCP) and packet[TCP].dport == 143 and "LOGIN" in payload.upper():
            ...
            if len(parts) >= 3:
                u, p = parts[-2], parts[-1]
                credentials["IMAP"].append(bcolors.BLUE + bcolors.BOLD + f"{ip} → LOGIN: "
                ...
                # Collect characters from client
                    ...
                    if len(telnet_payload) == 1 and telnet_payload.isprintable():
                        ...
                        if char_buffer[session]:
                            ...
                            print(bcolors.BLUE + bcolors.BOLD + f"[Client → Server] {ip_src}:{tcp.sport} → {ip_dst}:{tcp.dport}" + bcolors.ENDC)
                    ...
                    if '\r' in payload or '\n' in payload:
                        if char_buffer[session]:
                            if print_telnet_welcome:
                                print(bcolors.PURPLE + bcolors.BOLD + "[Telnet] Credentials:" + bcolors.ENDC)
                                print_telnet_welcome = False
                            print(bcolors.BLUE + bcolors.BOLD + f"[Client → Server] {ip_src}:{tcp.sport} → {ip_dst}:{tcp.dport}")
                            if telnet_state == "waiting_username":
                                print(bcolors.YELLOW + bcolors.BOLD + f"Username: " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{repr(char_buffer[session])}" + bcolors.ENDC)
                            elif telnet_state == "waiting_password":
                                print(bcolors.YELLOW + bcolors.BOLD + f"Password: " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{repr(char_buffer[session])}" + bcolors.ENDC)
        ...
        if packet.haslayer(TCP) and packet.haslayer(Raw):
            ...
            if "AUTH LOGIN" in payload_str or re.fullmatch(r"[A-Za-z0-9+/=]{6,}", payload_str):
                ...
                if not decoded or not decoded.isprintable() or len(decoded) < 3:
                    continue
                entry = bcolors.BLUE + bcolors.BOLD + f"{ip} → AUTH (Base64): " + bcolors.ENDC + bcolors.GREEN + bcolors.BOLD + f"{decoded}"
